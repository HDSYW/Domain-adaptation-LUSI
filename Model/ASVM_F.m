function [result] = ASVM_F(Data,K,ktype,pa)
test_ac=0;
k=K;
...
test_ac=0;
kernel1=pa.kernel1;
k = 3;  indices = crossvalind('Kfold',X_train_T(:,1),k);
for p1= pa.min:pa.step:pa.max
    Para.p1=2.^(p1);
    for power= pa.min:pa.step:pa.max
        Para.kpar.kp1 =2.^power; Para.kpar.kp2 = 0;
...
Para.p1=best_p1; Para.kpar.kp1=best_kp1;
svm=train_svm(Trn.X' ,Trn.Y' ,kernel1 ,best_kp1 ,best_p1 ,DV);
[R,~]=test_svm(svm,X_test_T',Y_test_T',kernel1,best_kp1,DV_L);
PredictY=R.Y';
% ---------- R ----------
