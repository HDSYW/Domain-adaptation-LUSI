function [decs,svm_model] = DASVM(pos_features, neg_features, ul_features, Para)
...
%     fprintf('\t\t C = %f,\t C* = %f.\n', sp.svm_C, sp.svm_Cu);
    ...
    % train SVM
    training_kernel = KerF(training_features', Para.kpar,training_features');
...
%         fprintf('Stop criteria reached, break.\n');
    ...
    % are fixed
    s_test_kernel = KerF(s_features(:, s_pool)',Para.kpar, training_features');
    u_test_kernel = KerF(ul_features',Para.kpar ,training_features');
...
%     fprintf('select %d pos_cand %d neg_cand...\n', length(pos_cand), length(neg_cand));
    ...
    Kerneltest=KerF(ul_features',Para.kpar,training_features' );
    ay      = full(svm_model.sv_coef)*svm_model.Label(1);
    idx     = full(svm_model.SVs);
    b       = -(svm_model.rho*svm_model.Label(1));
    decs    = sign(Kerneltest(:, idx)*ay + b);  
